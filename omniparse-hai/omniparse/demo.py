...
image_url_docs = {
    "curl": """curl -X GET -F "url: "https://example.com"} http://localhost:8000/parse_image/process_url""",
    "python": """
    coming soon⌛
    """,
    "javascript": """
...
def parse_image_url(url, request: gr.Request):
    ...
    try:
        ...
        # URL for image parsing
        get_url = f"http://{host_url}/parse_image/process_url?url={url}"
...
with demo_ui:
    ...
    with gr.Tabs():
        ...
        with gr.TabItem("Images"):
            with gr.Tabs():
                ...
                with gr.TabItem("Process image url"):
                    with gr.Tabs():
                        with gr.Row():
                            ...
                            with gr.Column(scale=200):
                                image_url_output_text = gr.Textbox(label="Output Text")
                                image_url_output_image = gr.Gallery(
                                    label="Output Image ⌛", interactive=False
                                )
                        with gr.Accordion("JSON Output"):
                            image_url_json = gr.JSON(label="Output JSON", visible=False)
                        with gr.Accordion("Use API", open=True):
                            gr.Code(
                                language="shell",
                                value=image_url_docs["curl"],
                                lines=1,
                                label="Curl",
                            )
                            gr.Code(
    ...
    image_process_button.click(
        fn=process_image,
        inputs=[image_uploaded, image_process_parameter],
        outputs=[
    ...
    url_button.click(
        ...
        inputs=[image_url],
        outputs=[
            image_url_output_text,
            image_url_output_image,
            image_url_json,
        ],
    )
